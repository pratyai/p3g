...
class TestProveExistsDataForallLoopBoundsIterIsdep:
    def test_data_aware_bi_dofs_forall_bounds(self):
        ...
        print("\n--- Generated SMT Query (data_aware_bi_dofs_forall_bounds) ---")
        print(smt_query)
        print("-------------------------------------------")

        # EXPECT: sat (True) - A data configuration exists (e.g., all B[i] > 0)
        # that forces sequential execution (A[i] = A[i-1] always executes).
        result = solve_smt_string(smt_query, "data_aware_bi_dofs_forall_bounds")
        assert result, (
