...
def test_cholesky_sequential():
    ...
    """
    print("--- Running Test: Cholesky Decomposition-like Kernel ---")
    b = GraphBuilder()
    ...
    with b.add_loop("L_outer", "i", Int(2), N,
                    reads=[(L_root, ((Int(1), N), (Int(1), Minus(N, Int(1)))))],
                    writes=[(L_root, ((Int(2), N), (Int(2), N)))]) as L_outer:
        i = L_outer.loop_var
        ...
        with b.add_loop("L_inner", "j", Int(2), i,
                        reads=[(L_local_outer, ((Int(1), i), (Int(1), Minus(i, Int(1)))))],
                        writes=[(L_local_outer, ((Int(2), i), (Int(2), i)))]) as L_inner:
            j = L_inner.loop_var
    ...
    # --- 1. Check Inner Loop (Expected: Sequential/UNSAT) ---
    print("\n-- 1. Checking Inner Loop (L_inner) for Parallelism --")
    # The inner loop end 'i' is a free variable for this check.
    smt_query_inner = generate_smt_for_prove_exists_data_forall_iter_isdep(inner_loop_node, outer_loop_node.loop_var)

    # EXPECT: unsat (False) - Inner loop has dependency L[i,j] <- L[i,j-1]
    result_inner = solve_smt_string(smt_query_inner, "cholesky_inner")
    assert result_inner
    print("\nInner Loop Verdict: Sequential (DOFS).")

    # --- 2. Check Outer Loop (Expected: Sequential/UNSAT) ---
    print("\n-- 2. Checking Outer Loop (L_outer) for Parallelism --")
    smt_query_outer = generate_smt_for_prove_exists_data_forall_iter_isdep(outer_loop_node, N)

    # EXPECT: unsat (False) - Outer loop has dependency through L[j-1,j-1]
    # e.g., when j=i, L[i,i] depends on L[i-1,i-1]
...
def test_cholesky_full_kernel():
    ...
    """
    print("--- Running Test: Full Cholesky Kernel ---")
    b = GraphBuilder()
    N = b.add_symbol("N", INT)
    ...
    with b.add_loop("L_outer", "i", Int(0), Minus(N, Int(1)),
                    reads=[(A_root, ((Int(0), N), (Int(0), N))),  # Reads entire A
                           (L_root, ((Int(0), N), (Int(0), N)))],  # Reads entire L
                    writes=[(L_root, ((Int(0), N), (Int(0), N)))]) as L_outer:  # Writes entire L
        i = L_outer.loop_var
        outer_loop_node = L_outer
        ...
        with b.add_loop("L_middle", "j", Int(0), i,
                        reads=[(A_local_outer, ((Int(0), Plus(i, Int(1))), (Int(0), Plus(i, Int(1))))),
                               # Reads A[0..i, 0..i]
                               (L_local_outer, ((Int(0), Plus(i, Int(1))), (Int(0), Plus(i, Int(1)))))],
                        # Reads L[0..i, 0..i]
                        writes=[(L_local_outer, ((Int(0), Plus(i, Int(1))),
                                                 (Int(0), Plus(i, Int(1)))))]) as L_middle:  # Writes L[0..i, 0..i]
            j = L_middle.loop_var
            ...
            # Scalar for sum_val
            sum_val_scalar = b.add_data("sum_val", is_output=False)  # Scalar, so no pysmt_array_sym

            # Initialize sum_val = 0 (as a compute node)
            b.add_compute("T_init_sum",
                          reads=[],
                          writes=[(sum_val_scalar, Int(0))]  # Write 0 to sum_val
                          )

            # Innermost loop: for k = 0 to j-1 (if j > 0)
            with b.add_loop("L_inner", "k", Int(0), Minus(j, Int(1)),
                            reads=[(L_local_middle, ((Int(0), Plus(i, Int(1))), (Int(0), Plus(j, Int(1))))),
                                   # Reads L[0..i, 0..j]
                                   (sum_val_scalar, Int(0))],  # Reads previous sum_val
                            writes=[(sum_val_scalar, Int(0))]) as L_inner:  # Writes new sum_val
                k = L_inner.loop_var
                inner_loop_node = L_inner
                ...
                b.add_compute("T_sum_accum",
                              reads=[(sum_val_local_inner, Int(0)),  # Read current sum_val
                                     (L_local_inner, (i, k)),  # Read L[i,k]
                                     (L_local_inner, (j, k))],  # Read L[j,k]
            ...
            b.add_compute("T_final_Lij",
                          reads=[(A_local_middle, (i, j)),  # Read A[i,j]
                                 (sum_val_scalar, Int(0)),  # Read final sum_val
                                 (L_local_middle, (j, j))],  # Read L[j,j] (for division, if modeled)
                          writes=[(L_local_middle, (i, j))]  # Write L[i,j]
                          )

    print_p3g_structure(b.root_graph)

    # --- Check Outer Loop (Expected: Sequential/UNSAT) ---
    print("\n-- Checking Full Cholesky Kernel (L_outer) for Parallelism --")
    smt_query_outer = generate_smt_for_prove_exists_data_forall_iter_isdep(outer_loop_node, N)

    # EXPECT: unsat (False) - Full Cholesky is highly sequential
    result_outer = solve_smt_string(smt_query_outer, "cholesky_full_kernel")
    assert\
        result_outer
    print("\nFull Cholesky Kernel Verdict: Sequential (DOFS). All checks PASSED.")
