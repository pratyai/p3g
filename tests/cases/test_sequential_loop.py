...
class TestProveExistsDataExistsLoopBoundsExistsIterIsdep:
    def test_sequential_loop_find_dependency(self):
        ...
        """
        print(
            "\n--- Running Test: Sequential Loop (Find Dependency) (Expected: SAT) ---"
        )
        b_root_graph, loop_node, N, A_root, B_root = build_sequential_loop_graph()
        ...
        print("\n--- Generated SMT Query (sequential_loop_find_dependency) ---")
        print(smt_query)
        print("---------------------------------------------")

        # EXPECT: sat (True) - A dependency should be found.
        result = solve_smt_string(smt_query, "sequential_loop_find_dependency")
        assert result, (
            "Expected to find a dependency for sequential loop but SMT solver returned UNSAT."
        )
        print("\nVerdict: PASSED. Found a dependency for Sequential Loop as expected.")
