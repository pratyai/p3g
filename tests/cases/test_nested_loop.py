...
def test_nested_loop_outer_dofs():
    ...
    with b.add_loop("L_outer", "i", Int(1), N,
                    reads=[(A_root, (Int(1), N)), (B_root, (Int(1), N))], # Outer loop accesses A[1..N, ...] and B[1..N, ...]
                    writes=[(A_root, (Int(1), N))]) as L_outer: # Outer loop writes A[1..N, ...]
        ...
        with b.add_loop("L_inner", "j", Int(1), M,
                        reads=[(A_local_outer, (Int(1), M)), (B_local_outer, (Int(1), M))], # Inner loop accesses A[..., 1..M] and B[..., 1..M]
                        writes=[(A_local_outer, (Int(1), M))]) as L_inner: # Inner loop writes A[..., 1..M]
    ...
    result_inner = solve_smt_string(smt_query_inner, "nested_loop_outer_dofs_inner_check")
    ...
    result_outer = solve_smt_string(smt_query_outer, "nested_loop_outer_dofs_outer_check")
...
def test_nested_loop_inner_dofs():
    ...
    with b.add_loop("L_outer", "i", Int(1), N,
                    reads=[(A_root, (Int(1), N)), (B_root, (Int(1), N))], # Outer loop accesses A[1..N, ...] and B[1..N, ...]
                    writes=[(A_root, (Int(1), N))]) as L_outer:
        i_sym = L_outer.loop_var
        loop_node = L_outer  # The highest level loop node for the SMT check
        ...
        with b.add_loop("L_inner", "j", Int(1), M,
                        reads=[(A_local_outer, (Int(1), M)), (B_local_outer, (Int(1), M))], # Inner loop accesses A[..., 1..M] and B[..., 1..M]
                        writes=[(A_local_outer, (Int(1), M))]) as L_inner:
            j_sym = L_inner.loop_var
            L_inner_node = L_inner
            ...
            # Use Tuple for 2D indexing
            reads_A = (A_local_inner, (i_sym, Minus(j_sym, Int(1)))) # Dependency on j-1
            reads_B = (B_local_inner, (i_sym, j_sym))
            writes_A = (A_local_inner, (i_sym, j_sym))
